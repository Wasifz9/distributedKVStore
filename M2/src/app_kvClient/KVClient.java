...
public class KVClient implements IKVClient {
		...
		try {
			...
			switch (res.getStatus()) {
				...
				case PUT_UPDATE:
					...
					break;
				case PUT_ERROR:
					printError("PUT operation with key " + res.getKey() + " failed: "
							+ res.getValue());
					break;
				case DELETE_SUCCESS:
	...
	private void printHelp() {
		...
		sb.append(
				"\t\t establishes a connection to the storage server based on the given server address and the port number of the storage service\n");
